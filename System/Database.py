...
import time
import os
from .Spider import spider,spider2,inSamePht
import caffe
...
###############################################################################
## Delete the images that we don't choose to reduce the storage that has     ##
## been used.                                                                ##
###############################################################################
...
###############################################################################
## Return the status of the DatabaseAppending in the database                ##
## Not used yet.                                                             ##
###############################################################################
def RebaseStatus():
    global databaseRebuilding
...
###############################################################################
## Set the tree status and fill in the information in the tree               ##
###############################################################################
...
###############################################################################
## Calculate the distance of features between two existing face in the       ##
## database.                                                                 ##
###############################################################################
...
###############################################################################
## Calculate the similarity between two features. Used with the former one   ##
## function.                                                                 ##
###############################################################################
def featureTransvection(feature0 ,feature1):
    return numpy.sum(numpy.dot(feature0,feature1.T))

###############################################################################
## Calculate the time in order to calculate the simliarity between two image ##
...
###############################################################################
## Combine the two existing infromation together to renew the information in ##
## the database.                                                             ##
###############################################################################
...
###############################################################################
## Initial and create an empty database for the whole system.                ##
## Used for the first time ONLY                                              ##
###############################################################################
def databaseInit():
...
###############################################################################
## Search the database to find the quite similarity feature in the database  ##
###############################################################################
...
###############################################################################
## Update the familiarity of the people in the database.                     ##
###############################################################################
...
###############################################################################
## Update the similarity about different people in the system via photos     ##
...
###############################################################################
## Append the information that has been collected by the camera.             ##
###############################################################################
...
###############################################################################
## Search the information where exists in the database or not and return to  ##
## the screen.                                                               ##
###############################################################################
...
###############################################################################
## Rebuild the whole system every night                                      ##
###############################################################################
def databaseRebuild():
    global databaseRebuilding
...
def databaseTest():
    ...
    for i in range(0,50):
        if (i > 0) and (random.randint(0,1) == 0):
            ...
            continue
        a = numpy.random.uniform(-1,1,512)
        s = numpy.sqrt(numpy.dot(a.T,a))
        a /= s
        info = dict()
...
###############################################################################
## Main function of the database. Second thread of the whole system.         ##
###############################################################################
def DatabaseBase():
    global databaseRebuilding
    databaseRebuilding = 0
    ...
    while True:
        ...
        if keyTime < 30000 and keyTime>23000:
