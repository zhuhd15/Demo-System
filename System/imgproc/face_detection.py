...
def FaceDetect(im,min_size,net):
    '''
    This function is used when a image is sent into this system.
    :param im: A RGB image that has been sent into this system(RGB ONLY!!!!!)
    :param min_size: the smallest size of the face area
    :param net: caffemodel
    :return: a 4-D list.detection area, as well as the extent picture
    '''
    M_BGR=[104.,117.,123.]
    ...
    if mx_bbox[3]-mx_bbox[1]<min_size:
        pass

    b_new=numpy.array([0,0,0,0])
    IterSum=0
    tete=Boundary[0,:,bbox[0]:bbox[2],bbox[1]:bbox[3]]
...
def feature_Extract(caffemodel,bbox,image,W,H):
    '''
    extract feature from the face area of the image
    :param caffemodel: recognize model
    :param bbox: area of face
    :param image: initial RGB picture
    :param W: width of out destination picture
    :param H: height of our destination picture
    :return: a 512-D vector of the FACE IMAGE that we sent in
    '''
...
def feature_Extract2(caffemodel,bbox,image,W,H):
    '''
    Same function as above, just use a second caffemodel
    :param caffemodel:
    :param bbox:
    :param image:
    :param W:
...
def face_Extract(caffemodel,bbox1,bbox2,image1,image2):
    '''
    compare the similarity of the two faces in two images
    :param caffemodel:
    :param bbox1:
    :param bbox2:
    :param image1:
